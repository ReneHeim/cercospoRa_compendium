...
### Read in data
```{r read_data}
cclo <- fread("../data/2025-12-02_fulldf_cc_cercospoRa.csv")[, "data" := "canopy_closure"]
eeeo <- fread("../data/2025-12-02_fulldf_eo_cercospoRa.csv")[, "data" := "epidemic_onset"]
```
...
We need to check the residuals for spatial autocorrelation.  

```{r}
library(spdep)

# Create a spatial weights matrix (e.g., based on k-nearest neighbors)
coords <- cbind(cclo$x, cclo$y)
knn_weights <- knearneigh(coords, k = 2) # k=2 for small dataset
nb_weights <- knn2nb(knn_weights)
listw <- nb2listw(nb_weights, style = "W", zero.policy = TRUE)

# Perform Moran's I test on OLS residuals
moran.test(residuals(mod), listw)
```

The moran test indicates a strong spatial auto-correlation. 
This indicates we should proceed with a model that can handle spatial autocorrelation.  

Generalised additive models are a reasonable fit here as our dependant variable
is an integer and it can handle non-linearity in the co-variates.
...
Lets also check if autocorrelation remains

```{r}
moran.test(residuals(mod1), listw)
```

Auto correlation is much less significant, but still significant.  
...
Differences in canopy closure dates between fungicide treated areas were 
`r interpret_gam(mod1a, words = 1, coeff = "fungicidetreated", sig = TRUE)` 
compared to untreated for date of canopy closure.  

Non-Inoculated areas showed 
`r interpret_gam(mod1a, words = 5, coeff = "inoculuminoculated")` canopy closure, 
of `r interpret_gam(mod1a, words = 0, coeff = "inoculuminoculated")` days 
compared to inoculated areas.  

Remote sensing platform UAS detected canopy closure 
`r interpret_gam(mod1a, words = 0, coeff = "sourceuas")` days 
`r interpret_gam(mod1a, words = 5, coeff = "sourceuas")`.  
...
Overall the interaction between fungicide and RS source was non significant.
...
UAS estimated canopy closure. an average of 
