<<<<<<<< HEAD:analysis/00_create_lai_maps.R
...
retrieve_lai <- function(image_date,
                         ...
                         data_resampling_matrix,    # data frame: sensor band centers/FWHM
                         band_index = c(2:5, 8),    # bands to use from the image
                         platform = "S2",           # label for output folder (e.g. "S2" or "UAV")
                         harmonized = FALSE,        # is image already harmonized (no offset)?
                         normalized = FALSE,        # is image already scaled to reflectance (0–1)?
                         aggregate = FALSE,         # aggregate (downscale) for UAV images?
                         agg_factor = 50,           # aggregation factor for spatial resolution
  ...
  # Base output folder for LAI maps
  lai_folder_directory <-  here::here("output", "LAI_maps")
  if (!dir.exists(lai_folder_directory)) dir.create(lai_folder_directory,recursive = TRUE)

  # Subfolder by platform (e.g. "S2", "UAV")
  output_foldername <- file.path(lai_folder_directory, platform)
  if (!dir.exists(output_foldername)) dir.create(output_foldername)

  # Full output file name: <output/LAI_maps2/platform/date.tif>
  output_filename <- file.path(output_foldername, paste0(image_date, ".tif"))
...
# Now to generate the LAI estimations and save the outputs

lapply(platforms, function(pform){

  # Get all the directories
  image_dirs <- list.dirs(here::here("data"), recursive = TRUE)
  # only return from folder with the platform name
  image_dir <- image_dirs[endsWith(image_dirs, pform$platform)]
  if(length(image_dir) != 1)stop("Cant locate the directroy with ",
                                 pform$platform," image files")
  image_list <- list.files(image_dir,full.names = TRUE)
  # remove xml files ext
  image_paths <- image_list[endsWith(image_list, ".tif")]
  image_dates <- tools::file_path_sans_ext(list.files(image_dir)[endsWith(list.files(image_dir), ".tif")])

  message("Start ", pform$platform," loop")
  lapply(seq_along(image_paths),function(image_n){
    retrieve_lai(image_date = image_dates[image_n],
                 image_path = image_paths[image_n],
                 roi = roi,
                 spect = spect,
                 data_resampling_matrix = pform$data_resampling_matrix,
                 band_index = pform$band_index,
                 platform = pform$platform,
           harmonized = pform$harmonized,
           normalized = pform$normalized,
           aggregate = pform$aggregate,
           agg_factor = pform$agg_factor)
  })

})

>>>>>>>> origin/Paul_revisions:analysis/00_RTM_LAI_estimation.R
